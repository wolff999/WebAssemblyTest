...
void AMyCharacter::OpenCloseAction(AActor* OpenableActor)
{
	...
	else
	{
		...
		{
			ServerConnection->AddToMessage("Open: " + OpenableActor->GetName() + ", Location: " + GetActorLocation().ToString());
		...
		{
			ServerConnection->AddToMessage("Close: " + OpenableActor->GetName() + "Location: " + GetActorLocation().ToString());
			GetStaticMesh(OpenableActor)->AddImpulse(-AppliedForce * OpenableActor->GetActorForwardVector());
...
void AMyCharacter::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	...
	InputComponent->BindAction("ExitToMenu", IE_Pressed, this, &AMyCharacter::ExitToMenu);
}
...
void AMyCharacter::SetServerConnection(AServerConnection * NewServerConnection)
{
	ServerConnection = NewServerConnection;
}

AServerConnection* AMyCharacter::GetServerConnection() {
...
void AMyCharacter::SpawnActor(AActor * a, UClass * u) {
	FName test = FName(TEXT("testActor"));
	FVector Location(0.0f, 0.0f, 50.0f);
	FRotator Rotation(0.0f, 0.0f, 0.0f);
	FActorSpawnParameters SpawnInfo;
	SpawnInfo.Template = a;
	AStaticMeshActor * NewActor = GetWorld()->SpawnActor<AStaticMeshActor>(Location, Rotation, SpawnInfo);
	ItemMap.Add(NewActor, EItemType::GeneralItem);
...
{
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::SanitizeFloat(MaxGraspLength));
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("click"));
	//PopUp.Broadcast(FString(HitObject.GetActor()->GetName()));
...
void AMyCharacter::PickToInventory(AActor* CurrentObject)
{
	...
	if (bRightHandSelected)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, "Name " + CurrentObject->GetName());
		PopUp.Broadcast(CurrentObject->GetActorLocation().ToString());
		ServerConnection->AddToMessage("R, PickUp, Name: " + CurrentObject->GetName() + ", Location: " + CurrentObject->GetActorLocation().ToString());
	...
	else
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, "Name " + CurrentObject->GetName());
		PopUp.Broadcast(CurrentObject->GetActorLocation().ToString());
		ServerConnection->AddToMessage("L, PickUp, Name: " + CurrentObject->GetName() + ", Location: " + CurrentObject->GetActorLocation().ToString());
		if (!ServerConnection) {
...
void AMyCharacter::DropFromInventory(AActor* CurrentObject, FHitResult HitSurface)
{
	...
	if (TwoHandSlot.Num())
	{
		...
		for (auto Iterator : TwoHandSlot)
		{
			...
			ServerConnection->AddToMessage("Drop:" + Iterator->GetName() + ", Location: " + Iterator->GetActorLocation().ToString());
		}
		TwoHandSlot.Empty();
...
{
	PopUp.Broadcast(FString(TEXT("rotate!")));
...
void AMyCharacter::ExitToMenu()
{
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("Z"));
